...
class DALLE3 extends Tool {
  constructor(fields = {}) {
    ...
    // 6. Always end any prompt with the extensions "--v 6.0 --fast" to make sure that you use version 6 of the model midjurney and on the fast mode.
    ...
    // MAKE SURE that the end result is the same given description prompt (language and grammer fixed) with 3 or 4 short descriptions (2 words) after it with ',' after each one, then end with the arguments "--v 6.0 --fast", and make sure you didn't add any new items to the image or change the wanted mean of it, You will get 4 images (not 2 images or 1 image) so make sure you know that`;
    this.schema = z.object({
      prompt: z
        .string()
        .max(4000)
        .describe(
          'A given description of the desired image, add short enhance descriptions after it, make sure you end the prompt with "--v 6.0 --fast".',
        ),
    });
  }
